...
inline void _elip(float x_cen, float y_cen, float radiomenor, float radiomayor) {
   BeginPath(hDCMem);
   Arc(hDCMem, int(x_cen - radiomenor), int(y_cen - radiomayor),
               int(x_cen + radiomenor), int(y_cen + radiomayor),
               int(x_cen - radiomenor), int(y_cen - radiomayor),
               int(x_cen - radiomenor), int(y_cen - radiomayor));
   EndPath(hDCMem);
}

void elipse(float x_cen, float y_cen, float radiomenor, float radiomayor) {
   HPEN hPen = CreatePen(PS_SOLID, 1, _color);
   HGDIOBJ orig = SelectObject(hDCMem, hPen);
   _elip(x_cen, y_cen, radiomenor, radiomayor);
   StrokePath(hDCMem);
   SelectObject(hDCMem, orig);
   DeleteObject(hPen);
}

void elipse_lleno(float x_cen, float y_cen, float radiomenor, float radiomayor) {
   HBRUSH hBrush = CreateSolidBrush(_color);
   HGDIOBJ orig = SelectObject(hDCMem, hBrush);
   _elip(x_cen, y_cen, radiomenor, radiomayor);
   FillPath(hDCMem);
...
void elipse(float x_cen, float y_cen, float radiomenor, float radiomayor) {
   float x = x_cen - radiomenor, y = y_cen - radiomayor;
   _lock();
   XDrawArc(_dsp, _buffer, _bufgc, x, y, 2*radiomenor, 2*radiomayor, 0, 360 * 64);
   _unlock();
}
...
void elipse_lleno(float x_cen, float y_cen, float radiomenor, float radiomayor) {
   float x = x_cen - radiomenor, y = y_cen - radiomayor;
   _lock();
   XFillArc(_dsp, _buffer, _bufgc, x, y, 2*radiomenor, 2*radiomayor, 0, 360 * 64);
   _unlock();
}
